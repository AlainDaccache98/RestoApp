...
class TableVisualizerForCancelOI extends JPanel {
	...
	private HashMap<Rectangle2D, Seat> Seats;
	private String TableDetails;
	private Table selectedTable;
	private Seat selectedSeat;
	private int firstVisibleTable;

	public TableVisualizerForCancelOI() {
		super();
		init();
	}
	...
	private void init() {
		...
		Seats = new HashMap<Rectangle2D, Seat>();
		selectedTable = null;
		selectedSeat = null;
		firstVisibleTable = 0;
			...
			public void mousePressed(MouseEvent e) {
				...
				for (Rectangle2D rectangle : rectangles) {
					if (rectangle.contains(x, y)) {
						...
						System.out.println("Clicked a "+selectedSeat.getClass().getName());

						break;
					}
				}
				for (Rectangle2D rectangle : seats) {
					if (rectangle.contains(x, y)) {
						selectedSeat = Seats.get(rectangle);
						System.out.println("Clicked a "+selectedSeat.getClass().getName());
						break;
					}
				}
